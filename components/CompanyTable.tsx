...
export function CompanyTable({ companies, onRefresh }: CompanyTableProps) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/config?category=profit_type').then(r => r.json()),
    ]).then(([statusData, typeData, profitData]) => {
      setStatusOptions(statusData.map((o: { value: string }) => o.value));
      setCompanyTypeOptions(typeData.map((o: { value: string }) => o.value));
      setProfitTypeOptions(profitData.map((o: { value: string }) => o.value));
    }).catch(() => {});
  }, []);
