...
/* ── Color maps matching the badge colors used throughout the app ──────── */

const STATUS_COLORS: Record<string, string> = {
  'Client':         'bg-yellow-100 text-yellow-800 border border-yellow-300',
  'Hot Prospect':   'bg-red-100 text-red-700 border border-red-300',
  'Interested':     'bg-green-100 text-green-700 border border-green-300',
  'Not Interested': 'bg-gray-800 text-white',
  'Unknown':        'bg-gray-100 text-gray-500',
};

const SENIORITY_COLORS: Record<string, string> = {
  'C-Suite':  'bg-blue-100 text-blue-800',
  'VP Level': 'bg-yellow-100 text-yellow-800',
  'Director': 'bg-gray-800 text-white',
  'Manager':  'bg-orange-100 text-orange-700',
  'Other':    'bg-gray-100 text-gray-500',
};

const COMPANY_TYPE_COLORS: Record<string, string> = {
  '3rd Party Operator': 'bg-[#0B3C62] text-white',
  'Owner/Operator':     'bg-[#1B76BC] text-white',
  'Capital Partner':    'bg-[#FFCB3F] text-yellow-900',
  'Vendor':             'bg-[#E7DED9] text-gray-700',
  'Partner':            'bg-emerald-100 text-emerald-700',
  'Other':              'bg-gray-100 text-gray-500',
};

const CATEGORY_COLOR_MAP: Record<string, Record<string, string>> = {
  status: STATUS_COLORS,
  seniority: SENIORITY_COLORS,
  company_type: COMPANY_TYPE_COLORS,
};

function ColorBadge({ category, value }: { category: string; value: string }) {
  const colorMap = CATEGORY_COLOR_MAP[category];
  if (!colorMap) return null;
  const cls = colorMap[value];
  if (!cls) return null;
  return (
    <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold leading-tight ${cls}`}>
      {value}
    </span>
  );
}
            ...
            >
              {editingId === opt.id ? (
                ...
                <>
                  <DragHandle />
                  <span
                    className="flex-1 text-sm text-gray-800 py-1.5 px-2 rounded hover:bg-gray-50 cursor-pointer flex items-center gap-2"
                    onClick={() => handleEdit(opt)}
                  >
                    <ColorBadge category={category} value={opt.value} />
                    {!CATEGORY_COLOR_MAP[category]?.[opt.value] && opt.value}
                  </span>
                  <button
                    type="button"
                    onClick={() => handleEdit(opt)}
