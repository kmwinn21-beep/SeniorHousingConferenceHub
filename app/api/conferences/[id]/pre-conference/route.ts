...
export async function GET(
  ...
  for (const row of xSocialRes.rows) xSocialSet.add(`${row.attendee_id}_${row.conference_id}`);

  const xNotesSet = new Set<string>();
  for (const row of xNotesRes.rows) xNotesSet.add(`${row.attendee_id}_${row.conference_id}`);
  ...
  function calcAttendeeHealth(aid: number): number {
    ...
    let totalDepth = 0, ghostCount = 0, totalFus = 0, completedFus = 0;
    for (const c of confs) {
      ...
      if (fuData) { totalFus += fuData.total_fus; completedFus += fuData.completed_fus; }
    }
    ...
    return Math.round(Math.max(0, Math.min(100, avgDepthScore * 0.60 + followUpScore * 0.30 - ghostPenalty * 0.10)));
  }

  const attendeeHealthMap = new Map<number, number>();
  for (const a of attendees) attendeeHealthMap.set(a.id as number, calcAttendeeHealth(a.id as number));
  ...
  for (const a of attendees) {
    ...
    if (!clientCompanyMap.has(companyId)) {
      clientCompanyMap.set(companyId, { companyId, companyName: String(a.company_name || ''), wse: a.wse != null ? Number(a.wse) : null, attendees: [] });
    }
    clientCompanyMap.get(companyId)!.attendees.push({ id: Number(a.id), firstName: String(a.first_name || ''), lastName: String(a.last_name || ''), title: a.title ? String(a.title) : null });
  }
  ...
  const landscape = {
    ...
    priorOverlapTypeLabel, priorOverlapCount: priorOverlapAttendees.length,
    priorOverlapAttendees: priorOverlapAttendees.map((a) => ({
      id: a.id, first_name: a.first_name, last_name: a.last_name, title: a.title,
      company_name: a.company_name, seniority: resolveSeniority(a.seniority, a.title),
  ...
  for (const a of attendees) {
    ...
    if (!icpCompanyMap.has(cid)) icpCompanyMap.set(cid, { id: cid, name: String(a.company_name || ''), company_type: (a.company_type as string) ?? null, attendeeList: [] });
    icpCompanyMap.get(cid)!.attendeeList.push(a);
  }
  const icpCompanies: Array<{ id: number; name: string; company_type: string | null; avgHealth: number; assigned_user_names: string[]; attendees: Array<{ id: unknown; first_name: unknown; last_name: unknown; title: unknown; seniority: string; health: number }> }> = [];
  icpCompanyMap.forEach((c) => {
    const scores = c.attendeeList.map((a) => attendeeHealthMap.get(a.id as number) ?? 0);
    const avg = scores.length > 0 ? Math.round(scores.reduce((s, v) => s + v, 0) / scores.length) : 0;
    icpCompanies.push({ id: c.id, name: c.name, company_type: c.company_type, avgHealth: avg, assigned_user_names: resolveUserIds(c.attendeeList[0]?.company_assigned_user), attendees: c.attendeeList.map((a) => ({ id: a.id, first_name: a.first_name, last_name: a.last_name, title: a.title, seniority: resolveSeniority(a.seniority, a.title), health: attendeeHealthMap.get(a.id as number) ?? 0 })) });
  });
  ...
  meetingsByRep.forEach((repMeetings) => {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      if (sorted[i].meeting_date === sorted[i + 1].meeting_date && sorted[i].meeting_time === sorted[i + 1].meeting_time) {
        conflictIds.add(sorted[i].id as number); conflictIds.add(sorted[i + 1].id as number);
      }
    }
  });
  const meetingsData = meetings.map((m) => ({
    ...
    location: m.location, scheduled_by: resolveIdList(m.scheduled_by, userNameMap) ?? (m.scheduled_by ? String(m.scheduled_by) : null),
    outcome: m.outcome, meeting_type: m.meeting_type, first_name: m.first_name, last_name: m.last_name,
    title: m.title, company_name: m.company_name, company_id: m.company_id, hasConflict: conflictIds.has(m.id as number),
  }));

  const guestListByEvent = new Map<number, Array<{ attendee_id: number; first_name: string; last_name: string; title: string | null; company_name: string | null; company_id: number | null; company_type: string | null; rsvp_status: string; assigned_user_names: string[] }>>();
  for (const row of socialRsvpsRes.rows) {
    ...
    guestListByEvent.get(eid)!.push({
      attendee_id: Number(row.attendee_id), first_name: String(row.first_name), last_name: String(row.last_name),
      title: row.title ? String(row.title) : null, company_name: row.company_name ? String(row.company_name) : null,
      company_id: row.company_id != null ? Number(row.company_id) : null,
      company_type: row.company_type ? String(row.company_type) : null,
      rsvp_status: String(row.rsvp_status), assigned_user_names: resolveUserIds(row.assigned_user),
    });
  }
  const socialEventsData = socialEvents.map((se) => ({
    ...
    attending_count: 0, declined_count: 0, guestList: guestListByEvent.get(Number(se.id)) ?? [],
  }));

  const companyDetailMap = new Map<number, { profit_type: string | null; entity_structure: string | null; wse: number | null; services: string | null; icp: string | null; company_status: string | null; assigned_user_names: string[]; website: string | null; company_type: string | null; company_name: string }>();
  for (const a of attendees) {
    const cid = a.company_id as number | null;
    if (!cid || companyDetailMap.has(cid)) continue;
    companyDetailMap.set(cid, { profit_type: (a.profit_type as string) ?? null, entity_structure: (a.entity_structure as string) ?? null, wse: a.wse != null ? Number(a.wse) : null, services: (a.services as string) ?? null, icp: (a.icp as string) ?? null, company_status: (a.company_status as string) ?? null, assigned_user_names: resolveUserIds(a.company_assigned_user), website: (a.website as string) ?? null, company_type: (a.company_type as string) ?? null, company_name: String(a.company_name || '') });
  }
  ...
  for (const rel of internalRels) {
    ...
    companyRelsMap.get(cid)!.push({ rep_names: resolveUserIds(rel.rep_ids), relationship_status: resolveIdList(rel.relationship_status, relStatusMap) ?? String(rel.relationship_status || ''), description: String(rel.description || '') });
  }
  ...
  repCompaniesMap.forEach((cids, rep) => {
    ...
    cids.forEach((cid) => {
      ...
      repCompanies.push({ company_id: cid, company_name: String(compAttendees[0]?.company_name ?? ''), company_type: (compAttendees[0]?.company_type as string) ?? null, relationship_status: rel ? (resolveIdList(rel.relationship_status, relStatusMap) ?? String(rel.relationship_status || '')) : null, description: (rel?.description as string) ?? null, profit_type: detail?.profit_type ?? null, entity_structure: detail?.entity_structure ?? null, wse: detail?.wse ?? null, services: detail?.services ?? null, icp: detail?.icp ?? null, company_status: detail?.company_status ?? null, assigned_user_names: detail?.assigned_user_names ?? [], website: detail?.website ?? null, internal_relationships: companyRelsMap.get(cid) ?? [], attendees: compAttendees.map((a) => ({ id: a.id, first_name: a.first_name, last_name: a.last_name, title: a.title, seniority: resolveSeniority(a.seniority, a.title), status: a.status, health: attendeeHealthMap.get(a.id as number) ?? 0 })), notes: notes.slice(0, 5).map((n) => ({ id: n.id, content: n.content, created_at: n.created_at, rep: n.rep, attendee_name: n.attendee_name, conference_name: n.conference_name })) });
    });
  ...
  const relationshipsData = internalRels.map((rel) => {
    ...
    return { id: rel.id, company_id: cid, company_name: String(compAttendees[0]?.company_name ?? ''), relationship_status: resolveIdList(rel.relationship_status, relStatusMap) ?? String(rel.relationship_status ?? ''), description: String(rel.description ?? ''), rep_names: resolveUserIds(rel.rep_ids), contact_names: contactNames, attendees: compAttendees.map((a) => ({ id: a.id, first_name: a.first_name, last_name: a.last_name, title: a.title, seniority: resolveSeniority(a.seniority, a.title), health: attendeeHealthMap.get(a.id as number) ?? 0 })), recentNotes: notes.slice(0, 3).map((n) => ({ id: n.id, content: n.content, created_at: n.created_at, rep: n.rep })) };
  });
